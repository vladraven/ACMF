...
# Used by empirical_validation.py, which calibrates via calibrate_country_proxy()/predict_from_theta()
# rather than the legacy 12-param ACMFObjective used above.
from .identifiability import simple_fim_diagnostics as _simple_fim_diagnostics


def score_candidate_observables_simple(data, theta, base_observables, candidate_observables) -> pd.DataFrame:
    base = _simple_fim_diagnostics(data, theta, base_observables)
    rows = []
    for c in candidate_observables:
        if c in base_observables:
            continue
        d = _simple_fim_diagnostics(data, theta, list(base_observables) + [c])
        rows.append({
            'candidate': c,
            'rank_gain': d['rank'] - base['rank'],
            'condition_gain': base['condition_number'] / d['condition_number'] if d['condition_number'] else 0,
            'min_eigenvalue_gain': d['min_eigenvalue'] - base['min_eigenvalue'],
            'score': (d['rank'] - base['rank']) * 1e6 + (d['min_eigenvalue'] - base['min_eigenvalue']),
        })
    if not rows:
        return pd.DataFrame(columns=['candidate', 'rank_gain', 'condition_gain', 'min_eigenvalue_gain', 'score'])
    return pd.DataFrame(rows).sort_values(['rank_gain', 'score'], ascending=[False, False]).reset_index(drop=True)

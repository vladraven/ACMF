...
from .core import default_params, rhs

def huber_loss(residual, delta=1.0):
    r=np.asarray(residual,dtype=float); a=np.abs(r); return float(np.mean(np.where(a<=delta,0.5*r*r,delta*(a-0.5*delta))))
...
class PriorSpec:
    ...
    def penalty(self,value,lower=None,upper=None):
        ...
        if k=='beta':
            ...
            u=float(np.clip((x-lower)/(upper-lower),1e-12,1-1e-12)); return float(self.weight*(-(self.a-1)*np.log(u)-(self.b-1)*np.log(1-u)))
        raise ValueError(f'Unsupported prior kind: {self.kind}')
def default_prior_specs(theta_names=None):
    priors={'alpha7':PriorSpec('lognormal',mu=np.log(0.5),sigma=0.8,weight=0.2),'K_g':PriorSpec('lognormal',mu=np.log(0.4),sigma=0.6,weight=0.2),'beta_neg':PriorSpec('lognormal',mu=np.log(0.2),sigma=0.6,weight=0.2),'NaturalDecay':PriorSpec('lognormal',mu=np.log(0.04),sigma=0.7,weight=0.2),'q1':PriorSpec('beta',a=2,b=2,weight=0.05),'q3':PriorSpec('beta',a=2,b=2,weight=0.05),'alpha1':PriorSpec('lognormal',mu=np.log(0.4),sigma=0.8,weight=0.2),'b1':PriorSpec('lognormal',mu=np.log(0.04),sigma=0.8,weight=0.2),'Ch0':PriorSpec('beta',a=2,b=2,weight=0.05),'M0':PriorSpec('beta',a=2,b=2,weight=0.05),'G0':PriorSpec('beta',a=2,b=2,weight=0.05),'R0':PriorSpec('beta',a=2,b=2,weight=0.05)}
...
class ACMFObjective:
    ...
    def __init__(self,data,config=None):
        self.t=np.asarray(data['t'],dtype=float); self.data={k:np.asarray(v,dtype=float) for k,v in data.items() if k!='t'}; self.config=config or LossConfig()
    ...
    def _theta_to_params(self,theta):
        p=default_params(); p.alpha7,p.K_g,p.beta_neg,p.NaturalDecay=theta[0],theta[1],theta[2],theta[3]; p.q1,p.q3,p.alpha1,p.b1=theta[4],theta[5],theta[6],theta[7]; return p
    def _initial_state(self,theta): return np.array([self.data.get('A',[0.3])[0],self.data.get('Prod',[0.4])[0],theta[8],theta[9],theta[10],0.3,self.data.get('Inst',[0.6])[0],theta[11],self.data.get('F',[2.0])[0],self.data.get('P',[500.0])[0]],dtype=float)
    @staticmethod
    def _project(x): y=np.asarray(x,dtype=float).copy(); y[:8]=np.clip(y[:8],0,1); y[8]=np.clip(y[8],0,4); y[9]=max(y[9],0.0); return y
    def _integrate(self,theta):
        p=self._theta_to_params(theta); x0=self._project(self._initial_state(theta)); t0,tf=float(self.t[0]),float(self.t[-1]); dt=min(0.5,(tf-t0)/max(len(self.t)*2,10)); n=int(np.ceil((tf-t0)/dt))+1; tt=np.linspace(t0,tf,n); tr=np.zeros((n,10)); tr[0]=x0
        for i in range(1,n):
        ...
        return out
    def prior_penalty(self,theta): return float(sum(self.config.priors[n].penalty(theta[i],*self.BOUNDS[i]) for i,n in enumerate(self.THETA_NAMES) if n in self.config.priors))
    def __call__(self,theta):
        ...
        for v in self.config.observed_vars:
            if v not in self.data: continue
            idx=self.config.var_index[v]; scale=self.var_scale.get(v,1.0); y=self.data[v]; s=tr[:,idx]
            loss+=huber_loss((y-s)/scale,self.config.delta_huber)+self.config.lambda_deriv*huber_loss((compute_derivative(y,self.t)-compute_derivative(s,self.t))/scale,self.config.delta_huber); n+=1
...
def differential_evolution_fit(objective,bounds=None,maxiter=5,popsize=15,workers=1,seed=42,tol=1e-6):
...
def estimate_covariance(hess_inv, objective=None):
    ...
    cov=np.asarray(hess_inv,dtype=float); d=np.sqrt(np.maximum(np.diag(cov),0)); corr=np.nan_to_num(cov/np.outer(d,d),nan=0.0,posinf=0.0,neginf=0.0); tri=corr[np.triu_indices_from(corr,k=1)]
    if tri.size and np.max(np.abs(tri))>0.85: warnings.warn(f"Multicollinearity detected: max|R|={np.max(np.abs(tri)):.3f}")
    return cov,corr
def dram_mcmc(objective,theta_map,cov_proposal=None,n_samples=200,burn_in=50,seed=42,**kwargs):
    rng=np.random.default_rng(seed); n=len(theta_map); cov_proposal=np.eye(n)*1e-4 if cov_proposal is None else np.asarray(cov_proposal); cur=np.asarray(theta_map,dtype=float).copy(); ce=0.5*objective(cur)**2; samples=np.zeros((n_samples,n)); acc=0
    for i in range(n_samples):
        prop=cur+rng.multivariate_normal(np.zeros(n),cov_proposal)
